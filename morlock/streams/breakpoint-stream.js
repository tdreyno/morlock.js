...
var { appendTo, apply, values, mapObj } = require('ramda');
var { getOption } = require('morlock/core/util');
var { testMQ } = require('morlock/core/dom');
...
function create(breakpoints, options) {
  ...
  var breakpointStreams = mapObj(function(val, key) {
    var s = Stream.create();
    ...
    return Stream.map(appendTo([key]), s);
  }, breakpoints);

  return apply(Stream.merge, values(breakpointStreams));
}
