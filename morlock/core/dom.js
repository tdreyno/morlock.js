...
var { indexOf, curry, flip, memoize, mapObj } = require('ramda');
var { isDefined, forEach } = require('morlock/core/util');
...
var cssPrefix = memoize(CustomModernizr.prefixed);

var setStyle = curry(function setStyle_(elem, key, value) {
  elem.style[cssPrefix(key)] = value;
});

function setStyles(elem, styles) {
  mapObj(flip(setStyle(elem)), styles);
}
...
var getStyle = curry(function getStyle_(elem, key) {
...
if (!isDefined(window.Element) || ('classList' in document.documentElement)) {
  ...
  hasClass_ = function hasClassPoly_(elem, className) {
    return indexOf(className, getClasses(elem)) !== -1;
  };
  ...
  removeClass_ = function removeClassPoly_(elem, className) {
    ...
    var idx = indexOf(className, currentClasses);
...
var hasClass = curry(hasClass_);
var addClass = curry(addClass_);
var removeClass = curry(removeClass_);

var addClasses = curry(function addClasses_(elem, classes) {
  forEach(addClass(elem), classes);
});

module.exports = {
  testMQ, hasClass, addClass, removeClass, addClasses,
  getViewportWidth, getViewportHeight, documentScrollY,
  getRect, cssPrefix, setStyle, setStyles,
  getStyle, insertBefore, detachElement, isVisible,
  getClasses
};
